...
pub fn start_sensor(
    ...
    thread::spawn(move || {
        ...
        // When the gap since the last successful cycle reaches 3, the safety system is notified via report_thermal_miss() for each missed cycle.
        ...
        loop {
            ...
            if next_release > now {
                if name == "Thermal" {
                    let remaining = next_release.duration_since(now);
                    // Only call sleep if there is more than 1.5ms left so we
                    // don't immediately overshoot via the OS wakeup latency.
                    if remaining > Duration::from_micros(1500) {
                        thread::sleep(remaining - Duration::from_micros(1500));
                    }
                    // Spin-wait for the final window to give sub-100µs precision on all platforms.
                    while Instant::now() < next_release {
                        std::hint::spin_loop();
                    }
                } else {
                    // Plain sleep in 5ms steps for Power (50ms) and Payload (100ms) since no strict jitter
            ...
            let lat_us = match buffer.push(data) {
                ...
                PushResult::Accepted => {
                    ...
                    if name == "Thermal" {
                        if value.is_nan() {
                            // Corrupted value counts as a miss
                            safety.report_thermal_miss(&metrics);
                        } else {
                            // Check whether any cycles were skipped since the last valid Thermal buffer insertion.
                            if thermal_last_ok > 0 {
                                let missed = cycle.saturating_sub(thermal_last_ok + 1);
                                // Fire one report_thermal_miss per missed cycle so the counter inside SafetySystem increments correctly and the alert fires at exactly 3.
                ...
                PushResult::Dropped(d) => {
                    ...
                    // Thermal has priority 0 (highest) so this should never execute (genuine miss and must be counted if does)
                    if name == "Thermal" {
                        safety.report_thermal_miss(&metrics);
